...
export const StudyManagePage = () => {
  ...
  const handleKick = async (targetUserId: number) => {
    try {
      await kickMember({ studyId: numericStudyId, targetUserId }).unwrap();
  ...
  const handleBlacklistAdd = async (targetUserId: number) => {
    const reason = window.prompt("블랙리스트 사유를 입력해 주세요.") ?? undefined;
    try {
      await addToBlacklist({ studyId: numericStudyId, targetUserId, reason }).unwrap();
